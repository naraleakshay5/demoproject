...
@Component({
  selector: 'app-aoi-recipe-setup',
  templateUrl: './aoi-recipe-setup.component.html',
  styleUrls: ['./aoi-recipe-setup.component.scss'],
})
export class AoiRecipeSetupComponent implements OnInit {
  machineId!: number;
  recipe: any;
  ...
  getLoadRecipe() {
    ...
    this.sharedService.getLoadRecipe(this.machineId).subscribe((res: any) => {
      ...
      if (this.recipeAlready && this.recipeAlready?.length > 0) {
        this.isSame = this.recipeAlready.every((ele: loadrecipe) => {
  ...
  proceedToCompleteMachineAndRecipeStup() {
    ...
    this.router.navigate(['op/aoi/dry-run']);
  }
}
