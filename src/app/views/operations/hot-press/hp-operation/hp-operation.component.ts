...
@Component({
  selector: 'app-hp-operation',
  templateUrl: './hp-operation.component.html',
  styleUrls: ['./hp-operation.component.scss'],
})
export class HpOperationComponent implements OnInit {
  poData!: PO_DATA;
  processId: any;
  progress: number = 99;
    ...
    this.sharedService.postTestResult(elements).subscribe({
      next: (res: any) => {
        if (this.isReworkBtn) {
          ...
          this.router.navigate(['op/hp/output-carrier-scan']);
        }
      },
      error: (error) => {
        console.info(error);
