...
@Component({
  selector: 'app-tp-process-quality',
  templateUrl: './tp-process-quality.component.html',
  styleUrls: ['./tp-process-quality.component.scss'],
})
export class TpProcessQualityComponent implements OnInit {
  poData!: PO_DATA;
  machineId: any;
  processId: any;
  visualInspect: any[] = [];
  ...
  proceed() {
    if (this.appStorage.get('IS_QUALITY_CHECK')) {
      this.router.navigate(['op/tp/operation']);
      this.appStorage.clear('IS_QUALITY_CHECK');
    } else {
      this.fqaService.testComplete('Pull Test');
