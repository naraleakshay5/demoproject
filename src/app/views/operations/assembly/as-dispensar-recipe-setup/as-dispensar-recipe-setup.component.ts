...
@Component({
  selector: 'app-as-dispensar-recipe-setup',
  templateUrl: './as-dispensar-recipe-setup.component.html',
  styleUrls: ['./as-dispensar-recipe-setup.component.scss'],
})
export class AsDispensarRecipeSetupComponent implements OnInit {
  turns: any = [];
  message: string = '';
  ...
  ngOnInit(): void {
    ...
    this.recipeAlready = this.appStorage.get('DISPENSER_RECIPE_LOADED_FOR_PO');
    ...
    this.machineData = this.machineData.find(
      (ele: any) => ele.is_primary == false && ele.name == '25-RESIN'
    );
      ...
      .subscribe({
        next: (response: any) => {
          if (response.success && response.op === 'LOAD_PRODUCT_RECIPE') {
            ...
            this.appStorage.set(
              'DISPENSER_RECIPE_LOADED_FOR_PO',
              response.nodes
            );
            this.isSuccess = true;
