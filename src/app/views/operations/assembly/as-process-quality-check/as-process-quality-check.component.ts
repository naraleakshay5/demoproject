...
@Component({
  selector: 'app-as-process-quality-check',
  templateUrl: './as-process-quality-check.component.html',
  styleUrls: ['./as-process-quality-check.component.scss'],
})
export class AsProcessQualityCheckComponent implements OnInit {
  wicPoData!: PO_DATA;
  machineId: any;
  processId: any;
  visualInspect: any[] = [];
  ...
  proceed() {
    if (this.isAllSampleCorrect == false) {
    ...
    const reqobj = {
      poid: this.wicPoData.po_id,
      sachid: this.wicPoData.sach_id,
      machineId: this.machineId,
      processId: this.processId,
      results: req,
    };
    this.sharedService
      .postSpcVisualResults(this.wicPoData?.po_id, reqobj)
      .subscribe();
  }
  ...
  failSamples() {
    let pitchSample = this.visualInspect.filter(
      (ele: any) => ele.parameter == 'Pitch'
    );
    pitchSample = pitchSample.map((ele) => {
      return {
        sample1: ele.sample1,
        sample2: ele.sample2,
        sample3: ele.sample3,
    ...
    pitchSample = Object.values(pitchSample[0]);
    this.countPitchSample = pitchSample.filter((ele: any) => ele === false);

    let dtSample = this.visualInspect.filter(
      (ele: any) => ele.parameter == 'D.T.'
    );
    dtSample = dtSample.map((ele) => {
      return {
        sample1: ele.sample1,
        sample2: ele.sample2,
        sample3: ele.sample3,
    ...
    dtSample = Object.values(dtSample[0]);
    this.countDTSample = dtSample.filter((ele: any) => ele === false);

    let weldingSample = this.visualInspect.filter(
      (ele: any) => ele.parameter == 'Welding in center'
    );
    weldingSample = weldingSample.map((ele) => {
      return {
        sample1: ele.sample1,
        sample2: ele.sample2,
        sample3: ele.sample3,
    ...
    weldingSample = Object.values(weldingSample[0]);
    this.countWeldingSample = weldingSample.filter((ele: any) => ele === false);
  }
}
